...
class ParallelRunner:
    ...
    def run(self, test_mode=False, **kwargs):
        ...
        while True:
            ...
            zs_n_agents = learner.fixed_encoder.zs(
                self.batch["obs"][:, self.t, :])  # self.batch["obs"]  ensor:bs,max_leg,n_agent,obs_dim  ；[:,self.t,:]
            ...
            #                                   test_mode=test_mode)
            actions = self.mac.select_actions(self.batch, t_ep=self.t, zs=True, target_zs=False, t_env=self.t_env, bs=envs_not_terminated,test_mode=test_mode)
            ...
            zsa_n_agents = learner.fixed_encoder.zsa(
                th.cat([self.batch["fixed_zs"][:, self.t, :], self.batch["actions"][:, self.t, :]],
                       dim=2))  # self.batch["zs"][:,self.t,:]  ensor:bs(1),n_agent,z_dim
            ...
            for idx, parent_conn in enumerate(parent_conns):
                if not terminated[idx]:
                    data = parent_conn.recv()  #youshenme?
                    ...
                    post_transition_data["next_obs"].append(data["obs"])
                    ...
                    # pre_transition_data["fixed_zs"].append(data["obs"])
            ...
            pre_transition_data_zs_list_all = []
            for n_agent_id in range(learner.n_agents):
                pre_transition_data_zs_list_all.append(
                    zs_n_agents[:, n_agent_id, :].tolist())  # list[list:z_dim]
                ...
                if (self.t_env + self.t - self.last_learn_T) / self.args.learn_interval >= 1.0:
                    ...
                    if buffer.can_sample(self.args.batch_size) and (buffer.episodes_in_buffer > getattr(self.args, "buffer_warmup", 0)):
                        ...
                        for _ in range(getattr(self.args, "n_train", 1)):
                            learner.train(episode_sample, self.t_env, episode)#, self.args.nr_of_steps)
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            ...
            next_state, reward, terminated, env_info = env.step(actions)
