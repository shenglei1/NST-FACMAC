...
class ParallelRunner1:
    ...
    def run(self, test_mode=False, **kwargs):
        ...
        while True:
            ...
            post_transition_data['future_discounted_return'] = []
            ...
            for idx, parent_conn in enumerate(parent_conns):
                ...
                # # compute episode returns
                    if self.args.res:
                        # episode_batch_rewards =list [episode_returns[idx]]
                        # print(episode_batch_rewards.shape)

                        episode_batch_future_discounted_returns = 0.

                        future_discounted_return = data["reward"] + self.args.gamma * future_discounted_return
                        episode_batch_future_discounted_returns+= future_discounted_return

                        # episode_batch_future_discounted_returns =
                        data['future_discounted_return'] = episode_batch_future_discounted_returns
                        post_transition_data['future_discounted_return'].append(data['future_discounted_return'])
                ...
                if (self.t_env + self.t - self.last_learn_T) / self.args.learn_interval >= 1.0:
                    ...
                    if buffer.can_sample(self.args.batch_size) and (buffer.episodes_in_buffer > getattr(self.args, "buffer_warmup", 0)):
                        ...
                        for _ in range(getattr(self.args, "n_train", 1)):
                            learner.train(episode_sample.to(device), self.t_env, episode.to(device), self.args.nr_of_steps)
