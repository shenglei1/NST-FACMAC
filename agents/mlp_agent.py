...
class MLPAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.fc1 = nn.Linear(self.state_shape, args.rnn_hidden_dim)
        self.ln1 = nn.LayerNorm(args.rnn_hidden_dim)

        self.fc2 = nn.Linear(args.rnn_hidden_dim+self.args.zs_dim, args.rnn_hidden_dim)
    ...
    def forward(self, inputs, hidden_state, actions=None):
        x = self.fc1(inputs[:,:self.state_shape].view(-1, self.state_shape))
        x = AvgL1Norm(x)
