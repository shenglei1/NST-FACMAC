...
from src.modules.critics.facmac import FACMACDiscreteCritic
...
from src.modules.mixers.qmix_ablations import VDNState, QMixerNonmonotonic
...
class FACMACDiscreteLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # print("target_mac_out.shape", target_mac_out.shape)
        ...
        #     act_outs = self.mac.forward(batch, t=t)
        #     actor_mac_out.append(act_outs)
        # actor_mac_out = th.stack(actor_mac_out, dim=1)  # Concat over time
        #
        # chosen_action_qvals_act = actor_mac_out.max(dim=3)[0]
        # if self.mixer is not None:
        #     if self.args.mixer == "vdn":
        ...
        #     else:
        #         chosen_action_qvals_act = self.mixer(chosen_action_qvals_act, batch["state"][:, :-1])
        #
        # # Compute the actor loss
        # pg_loss = - (chosen_action_qvals_act * mask).sum() / mask.sum()
        #
        # # Optimise agents
