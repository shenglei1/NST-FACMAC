...
from modules.critics.facmac_normalq import FACMACCritic_nomalq
import torch as th
...
class FACMACLearnerNogasi:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.critic = FACMACCritic_nomalq(scheme, args)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, nr_of_steps):
        ...
        for _ in range(nr_of_steps):
            ...
            # print("action_shape",actions.shape) #[100, 1, n_agents, n_actions]
            ...
            if total_it % self.policy_freq == 0:
                ...
                for t in range(batch.max_seq_length):
                    ...
                    q=q.view(batch.batch_size, -1, 1) #[100, 4, 1]

                    chosen_action_qvals_ind.append(q)

                    mac_out.append(agent_outs)  #list[tensor]
                    if self.mixer is not None:
                ...
                distance_diverse = torch.clamp(distance_diverse, -1, 1)
                # Compute the actor loss
                alpha = alpha * phi
                print("chosen_action_qvals.mean()", chosen_action_qvals.mean())
                pg_loss = -chosen_action_qvals.mean() + (pi ** 2).mean() * 1e-3 - alpha * distance_diverse.mean()
