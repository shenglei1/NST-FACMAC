...
from modules.critics.facmac_sz import FACMACCritic_sz
import torch as th
...
class FACMACLearner_sz:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.encoder = Encoder(scheme, args)
        self.fixed_encoder = copy.deepcopy(self.encoder)
        self.fixed_encoder_target = copy.deepcopy(self.encoder)
        self.encoder_params = list(self.encoder.parameters())
        # self.encoder_params = self.encoder.parameters()
        self.encoder_optimizer = Adam(params=self.encoder_params, lr=args.lr,
                                      eps=getattr(args, "optimizer_epsilon", 10E-8))

        self.mac = mac
        self.target_mac = copy.deepcopy(self.mac)
        self.agent_params = mac.parameters()

        self.critic = FACMACCritic_sz(scheme, args)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        actions = batch["actions"][:, :-1] #[bs, maxlegth-1, n_agent,dim]
        terminated = batch["terminated"][:, :-1].float()
        mask = batch["filled"][:, :-1].float()
        mask[:, 1:] = mask[:, 1:] * (1 - terminated[:, :-1])
        # batch_Q_all = batch["batch_Q_all"][:, :-1]
        state = batch["state"][:,:-1]
        obs = batch["obs"] #[bs, maxlegth(26), n_agent,dim]
        next_obs = batch["next_obs"]
        zsa = batch["zsa"][:,:-1]
        target_zs = batch["fixed_target_zs"]
        #########################
        # Update Encoder
        #########################
        with torch.no_grad():
            next_zs = self.encoder.zs(next_obs[:,:-1])  #bs,batch.max_seq_length-1,n_agent,zs_ndim

        zs = self.encoder.zs(obs[:,:-1])
        pred_zs = self.encoder.zsa(th.cat([zs, actions],dim=3))
        ...
        with torch.no_grad():
            ...
            fixed_target_zs = self.fixed_encoder_target.zs(obs)  #bs,batch.max_seq_length,n_agent,zs_ndim(64)
            batch["fixed_target_zs"][:,:,:]= fixed_target_zs
            for t in range(batch.max_seq_length):  #batch.max_seq_length:
                agent_outs_zs_target = self.target_mac.select_actions(batch, t_ep=t, zs=False,target_zs=True,t_env=None, test_mode=True,
                                                                   critic=self.target_critic, target_mac=True)
                target_agent_out.append(agent_outs_zs_target)
            target_agent_out = th.stack(target_agent_out,dim=1)  #bs,max_seq_length,n_agent,action_dim


            target_vals = []
            self.target_critic.init_hidden(batch.batch_size)
            fixed_target_zsa = self.fixed_encoder_target.zsa(th.cat([batch["fixed_target_zs"], target_agent_out], dim=3))
            for t in range(1, batch.max_seq_length):
                # target_agent_out_3dim = th.squeeze(target_agent_out[:, t:t + 1, :])
                # fixed_target_zsa = self.fixed_encoder_target.zsa(
                #     th.cat([batch["fixed_target_zs"][:, t,:], target_agent_out_3dim], dim=2))
                target_inputs = self._build_inputs(batch, zs=False,taget_zs=True, t=t)#[bs*n_agent,ob_dim+z_dim]
                target_critic_out, \
                self.target_critic.hidden_states = self.target_critic(target_inputs, target_agent_out[:, t:t + 1].detach(),
                                                                      fixed_target_zsa[:, t:t + 1].detach(),self.target_critic.hidden_states)
                # self.target_critic.hidden_states = self.target_critic(target_inputs,
                #                                                       target_agent_out[:, t:t + 1].detach(),
            ...
            fixed_zs = self.fixed_encoder.zs(obs)  # bs,batch.max_seq_length,n_agent,zs_ndim(64)
            batch["fixed_zs"][:, :, :] = fixed_zs
            fixed_zsa = self.fixed_encoder.zsa(th.cat([batch["fixed_zs"][:, :-1], actions], dim=3))
        ...
        for t in range(batch.max_seq_length - 1):
            ...
            inputs = self._build_inputs(batch, zs=True,taget_zs=False,t=t)
            critic_out, self.critic.hidden_states = self.critic(inputs, actions[:, t:t + 1].detach(),fixed_zsa[:, t:t + 1].detach(),
                                                                self.critic.hidden_states)
            if self.mixer is not None:
                critic_out = self.mixer(critic_out.view(batch.batch_size, -1, 1), batch["state"][:, t:t + 1])  #[bs,1,1]

            # critic_out_prod = th.softmax(critic_out, dim=0) #[32/bs,1,1]
            # q_taken_prod.append(critic_out_prod)
            q_taken.append(critic_out)
            # if t == 0:
            #     KL_loss = th.nn.functional.kl_div(th.log(critic_out_prod), critic_out_prod)
            # else:
            #     KL_loss = th.nn.functional.kl_div(th.log(critic_out_prod), q_taken_prod[t-1])
            # # # Q_taken.append(critic_out)
            # KL_loss_all.append(KL_loss)

        q_taken = th.stack(q_taken, dim=1)
        # KL_loss_all_tensor = th.stack(KL_loss_all)
        # q_taken_prod = th.log_softmax(q_taken.squeeze(dim=2), dim=1)
        # Q_taken_prod = th.softmax(batch_Q_all, dim=1)
        ...
        loss = (masked_td_error ** 2).sum() / mask.sum() #- KL_loss_all_tensor.sum()
        ...
        for t in range(batch.max_seq_length):
            agent_outs = self.mac.forward(batch, t=t, zs=True, target_zs=False,select_actions=True)["actions"].view(batch.batch_size,
                                                                                           self.n_agents,
                                                                                           self.n_actions)
            fixed_zsa = self.fixed_encoder.zsa(th.cat([batch["fixed_zs"][:, t,:], agent_outs], dim=-1))
            q, self.critic.hidden_states = self.critic(self._build_inputs(batch, zs=True,taget_zs=False,t=t), agent_outs.detach(),fixed_zsa.detach(),
                                                       self.critic.hidden_states)
            if self.mixer is not None:
                q = self.mixer(q.view(batch.batch_size, -1, 1), batch["state"][:, t:t+1])
            mac_out.append(agent_outs)  #st[tensor[100,2,3]]
            chosen_action_qvals.append(q)
        # pdist = nn.PairwiseDistance(p=2)
        pi = th.stack(mac_out[:-1], dim=1)
        chosen_action_qvals = th.stack(chosen_action_qvals[:-1], dim=1)
        # pi = mac_out
        # distance_diverse = pdist(actions, mac_out)
        # distance_diverse = torch.clamp(distance_diverse, -0.2, 0.2)
        # # Compute the actor loss
        # alpha = alpha * phi
        # print("chosen_action_qvals.mean()",chosen_action_qvals.mean())
        # pg_loss = -chosen_action_qvals.mean()  - alpha * distance_diverse.mean()
    ...
    def _update_targets_soft(self, tau):
        ...
        for target_param, param in zip(self.fixed_encoder_target.parameters(), self.fixed_encoder.parameters()):
            target_param.data.copy_(target_param.data * (1.0 - tau) + param.data * tau)

        for target_param, param in zip(self.fixed_encoder.parameters(), self.encoder.parameters()):
            target_param.data.copy_(target_param.data * (1.0 - tau) + param.data * tau)
    ...
    def _build_inputs(self, batch,zs,taget_zs, t):
        bs = batch.batch_size
        inputs = []
        ...
        if self.args.recurrent_critic:
            ...
            if zs:
                inputs.append(batch["fixed_zs"][:, t])
            if taget_zs:
                inputs.append(batch["fixed_target_zs"][:, t])
