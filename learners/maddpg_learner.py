    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        with torch.no_grad():
            ...
            for t in range(batch.max_seq_length):
                agent_target_outs = self.target_mac.select_actions(batch, t_ep=t, zs=False,target_zs=True,t_env=None, test_mode=True,
            ...
            batch["zsa"][:, :-1] = fixed_zsa

            target_vals = []
            fixed_target_zsa = self.fixed_encoder_target.zsa(th.cat([batch["fixed_target_zs"], target_actions], dim=3))

            for t in range(1, batch.max_seq_length):
                target_inputs = self._build_inputs(batch, zs=False, taget_zs=True, t=t)
                target_critic_out, _ = self.target_critic(target_inputs, target_actions[:, t:t + 1].detach(),
                                                          fixed_target_zsa[:, t:t + 1].detach())
        ...
        for t in range(batch.max_seq_length - 1):
            ...
            critic_out, _ = self.critic(inputs, actions[:, t:t+1].detach(),fixed_zsa[:, t:t + 1].detach())
        ...
        for t in range(batch.max_seq_length - 1):
            ...
            for idx in range(self.n_agents):
                fixed_zsa_tem = batch["zsa"][:, :-1][:,t:t+1].detach().clone().view(batch.batch_size, -1, self.args.zs_dim)
                tem_joint_act = actions[:, t:t+1].detach().clone().view(batch.batch_size, -1, self.n_actions)
                tem_joint_act[:, idx] = agent_outs[:, idx]
                fixed_zsa_tem[:, idx] = self.fixed_encoder.zsa(th.cat([batch["fixed_zs"][:, t,idx], agent_outs[:, idx]], dim=-1))
                q, _ = self.critic(self._build_inputs(batch, zs=False,taget_zs=True, t=t), tem_joint_act,fixed_zsa_tem)
